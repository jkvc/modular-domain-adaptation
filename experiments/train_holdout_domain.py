...
def main(config: OmegaConf):
    ...
    for holdout_domain in train_collection.domain_strs:
        logger.info(f"running experiment holding out domain [{holdout_domain}]")
        use_domain_strs = [
            d for d in train_collection.domain_strs if d != holdout_domain
        ]
        logger.info(f"use domain {use_domain_strs}")
        ...
        train_dataset: MultiDomainDataset = DATASET_REGISTRY.from_config(
            ...
            use_domain_strs=use_domain_strs,
        )
        logger.info(
